...
def _preset_to_range(preset: DatePreset, now: datetime | None = None) -> tuple[date, date] | None:
...
def resolve_date_range(
    *,
    ...
    if date_preset:
        ...
        if rng is None:
            raise ValueError("'lifetime' is not supported for explicit date ranges in this pipeline")
        s, u = rng
        return DateRange(since=s, until=u)
    if since and until:
        return DateRange(since=date.fromisoformat(since), until=date.fromisoformat(until))
    if since or until:
        raise ValueError("Both --since and --until must be provided if not using --date-preset")
...
def sync_meta_insights(
    *,
    ...
    for ir in adapter.fetch_insights(level=level, account_id=act, date_range=dr, page_size=500):
        raw = ir.raw or {}
        ad_id = raw.get("ad_id")
        ...
        rows_bq.append({
            ...
            "campaign_global_id": f"meta:campaign:{campaign_id}" if campaign_id else None,
    ...
    if rows_bq:
        load_json_rows(project_id=project_id, dataset=dataset, table=INSIGHTS_TABLE, rows=rows_bq)
